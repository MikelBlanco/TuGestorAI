...
public class NumeracionService {
    ...
    /**
     * Devuelve el siguiente número de presupuesto para un autónomo en el año en curso.
     *
     * @param autonomoId ID del autónomo
     * @return número con formato {@code P-AAAA-NNNN}
     */
    public String siguienteNumeroPresupuesto(long autonomoId) {
        int anio = Year.now().getValue();
        int siguiente = presupuestoDao.contarPorAutonomoYAnio(autonomoId, anio) + 1;
        return String.format("P-%d-%04d", anio, siguiente);
    }

    /**
     * Devuelve el siguiente número de factura para un autónomo en el año en curso.
     * Las facturas deben ser correlativas sin saltos (requisito fiscal español).
     *
     * @param autonomoId ID del autónomo
     * @return número con formato {@code F-AAAA-NNNN}
     */
    public String siguienteNumeroFactura(long autonomoId) {
        int anio = Year.now().getValue();
        int siguiente = facturaDao.contarPorAutonomoYAnio(autonomoId, anio) + 1;
